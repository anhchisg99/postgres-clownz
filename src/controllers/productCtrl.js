...
const createProduct = async (req, res) => {
  ...
  if (!req.body) {
    ...
    throw new Error("not found req.body");
  }
  const { name, description, price, category_id } = req.body;
  // console.log(req.file)
  // let img = req.file.path
  const results = await productService.createProductService({
    name,
    description,
    price,
    category_id,
...
const getProductsBasedOnCategory = async (req, res) => {
  ...
  const results =
    await productService.getProductsBasedOnCategoryService(category_id);
  res.status(201).json({
    status: "success",
    results,
  });
...
const uploadImg = async (req, res) => {
  ...
  const results = await productService.uploadImgService({
